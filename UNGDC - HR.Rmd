...
ModelHHRR_H <- lm (HHRR ~ PTS_H^2 + PTS_H_var + PTS_H*PTS_H_var, data = merge_HHRRcounts)
#PTS_H insignificant
...
```{r}
ModelHHRR_S_fe <- plm (HHRR ~ factor(PTS_S) + PTS_S_var + factor(PTS_S)*PTS_S_var, data = merge_HHRRcounts, index = c("MergeCountry", "Year"), effect = "twoways", model = "within")
...
```{r}
aaa_HHRR<-ggplot(subset(plot_HHRR,!is.na(PTS_A_var)), aes(factor(PTS_A), HHRR))+
  geom_boxplot(varwidth=T, aes(fill=factor(PTS_A_var)))+
scale_fill_discrete(name = "Change in PTS Score", labels=c("No","Yes"))+
          labs(title="Human Rights Mentions by PTS Score", 
             subtitle="Baseline", 
             caption="", 
             y="Human Rights Mentions",
...
###Model Regression with sophisticated human rights dictionary
### Create Human Rights Dictionary
...
```{r}
ModelHRdict_S <- lm (HR ~ PTS_S + PTS_S_var + PTS_S*PTS_S_var, data = merge_HRdict_counts)

ModelHRdict_A <- lm (HR ~ factor(PTS_A) + PTS_A_var + factor(PTS_A)*PTS_A_var, data = merge_HRdict_counts)

ModelHRdict_H <- lm (HR ~ PTS_H + PTS_H_var + PTS_H*PTS_H_var, data = merge_HRdict_counts)
```
...
Plot the counts in HHRR mentions and the change in PTS by PTS score level
```{r}
ggplot(plot_HR.dict, aes(x=HR, y=PTS_A_var)) +
  geom_point()
```
...
aaa.1<-ggplot(subset(plot_HR.dict,!is.na(PTS_A_var)), aes(PTS_A, HR))+
  geom_boxplot(varwidth=T, aes(fill=factor(PTS_A_var)))+scale_fill_discrete(name = "Change in PTS Score", labels=c("No","Yes"))+
          labs(title="Human Rights Mentions by PTS Score", 
             subtitle="Sophisticated Dictionary", 
             caption="", 
             y="Human Rights Mentions",
             x="PTS Score - Amnesty International")+
        theme(axis.line = element_blank(),
...
```{r}
Binary_ModelHHRR_A <- glm (HHRR ~ factor(PTS_A) + PTS_A_var + factor(PTS_A)*PTS_A_var, family = binomial(link = "logit"), data = binary_HHRRcounts)
...
Running the Binary Model for the Amnesty International PTS database
```{r}
Binary_ModelHRdict_A <- glm (HR ~ factor(PTS_A) + PTS_A_var + factor(PTS_A)*PTS_A_var, family = binomial(link = "logit"), data = binary_HRdict_counts)
...
merge_HRdict_counts$regression<-ModelHRdict_A$coefficients["(Intercept)"]+ ModelHRdict_A$coefficients[2]*merge_HRdict_counts$PTS_A^2+ModelHRdict_A$coefficients[3]*merge_HRdict_counts$PTS_A_var+ModelHRdict_A$coefficients[4]*merge_HRdict_counts$PTS_A*merge_HRdict_counts$PTS_A_var
```

```{r}
prediction_model <- ggplot(merge_HRdict_counts,aes(year, MergeCountry)) + 
scale_x_continuous(limits=c(1976,2018),breaks=seq(1976,2018,21),expand = c(0, 0))+
  geom_line(aes(x=Year, y=HR, colour ="Mentions"))+
  geom_line(aes(x=Year, y=regression))+
    facet_wrap(~MergeCountry,ncol = 15) 
```

```{r}
prediction <- ggplot(subset(merge_HRdict_counts, MergeCountry=="USA")) + 
scale_x_continuous(limits=c(1976,2018),breaks=seq(1976,2018,21),expand = c(0, 0))+
  geom_line(aes(x=Year, y=HR, colour ="Mentions"))+
  geom_line(aes(x=Year, y=regression))+
    facet_wrap(~MergeCountry,ncol = 15) 
...
```{r}
a<-merge_HRdict_counts %>%
  subset(MergeCountry=="COL")
str(a)
...
```{r}
stargazer(ModelHHRR_A, ModelHHRR_A_fe, ModelHRdict_A, ModelHR_A_fe, align = TRUE,
          column.sep.width = "1 pt",
          title = "Table 4. Regresion Models Baseline and Sophisticated Dictionary Model",
          covariate.labels = c("PTS 2", "PTS 3", "PTS 4", "PTS 5", "Variance in PTS", "Interaction PTS2 and VAR", "Interaction PTS3 and VAR", "Interaction PTS4 and VAR"),
          dep.var.labels = c("Baseline","Sophisticated Dictionary"),
          dep.var.caption = "",
          model.names = FALSE,
          model.numbers = FALSE,
          column.labels = c("OLS", "Fixed Effects", "OLS", "Fixed Effects"),
...
```{r}
var.test(ModelHHRR_A, ModelHRdict_A)
```
