...
```{r}
ModelHHRR_S <- lm (HHRR ~ factor(PTS_S^2) + PTS_S_var + factor(PTS_S)*PTS_S_var, data = merge_HHRRcounts)
...
ModelHHRR_A_fe <- plm (HHRR ~ PTS_A + PTS_A_ch + PTS_A*PTS_A_ch, data = merge_HHRRcounts, index = c("MergeCountry", "Year"), effect = "twoways", model = "within")

screenreg(list(ModelHHRR_S_fe, ModelHHRR_S))
...
```{r}
aaa<-ggplot(subset(plot_HHRR,!is.na(PTS_A_ch)), aes(factor(PTS_A_ch), HHRR))+
  geom_boxplot(varwidth=T, aes(fill=factor(PTS_A)))
...
ModelHRdict_A <- lm (HR ~ PTS_A^2 + PTS_A_var + PTS_A*PTS_A_var, data = merge_HRdict_counts)
...
```{r}
ModelHR_A_fe <- plm (HR ~ PTS_A + PTS_A_ch + PTS_A*PTS_A_ch, data = merge_HRdict_counts, index = c("MergeCountry", "Year"), effect = "twoways", model = "within")
```
...
```{r}
aaa.1<-ggplot(subset(plot_HR.dict,!is.na(PTS_A)), aes(factor(PTS_A), HR))+
...
```{r}
bbb<- ggplot(subset(plot_HR.dict,!is.na(PTS_A_var)), aes(PTS_A_var))+
    geom_density(aes(fill=factor(PTS_A)), alpha=0.2)
```
